 ...
 * 历史开奖
 */
public class MyYuCeDetailActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.my_yuce_detail);
        InitView();
        tv_header_title.setText("预测详情");
        if (loadingDialog == null) {
            loadingDialog = new LoadingDialog(MyYuCeDetailActivity.this, R.style.LoadingDialog);
        }
        loadingDialog.show();
        app = new AppSp();
        userBean = (UserBean) app.getObjectFromShare(MyYuCeDetailActivity.this, "user");
        id = getIntent().getIntExtra("id", -1);
        tv_name.setText(userBean.getNicheng());
        tv_beizhu.setText(userBean.getBeizhu());
        img_user_url = userBean.getTx();
        if (userBean.getJifen()!=null) {
            tv_fensi.setText("积分:" + userBean.getJifen());
        }else{
            tv_fensi.setText("积分:0");
        }
        Glide.with(MyYuCeDetailActivity.this).load("http://www.zse6.com"+img_user_url).centerCrop().placeholder(R.drawable.img_user_tx)
                .transform(new GlideCircleTransform(MyYuCeDetailActivity.this))
    ...
    private void getDate(int id, int uid, String caizhong) {
        OkHttpUtils
            ...
            public void onError(okhttp3.Call call, Exception e, int id) {
                MyToast.getToast(MyYuCeDetailActivity.this, "请求失败");
            }
    ...
    public Handler myHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 200:
                    Glide.with(MyYuCeDetailActivity.this).load(img_user_url).centerCrop().placeholder(R.color.white)
                            .transform(new GlideCircleTransform(MyYuCeDetailActivity.this))
                            .diskCacheStrategy(DiskCacheStrategy.SOURCE).into(img_user);
                    ...
                    break;
                case 300:
                    ToastUtil.showToast(MyYuCeDetailActivity.this, "无数据");
                    if (loadingDialog != null) {
                        loadingDialog.cancel();
                    }
                    break;
                case 400:
                    ToastUtil.showToast(MyYuCeDetailActivity.this, "关注成功");
                    if (loadingDialog != null) {
                        loadingDialog.cancel();
                    }
                    break;
                case 100:
                    ...
                    //前面的lSysTime是秒数，先乘1000得到毫秒数，再转为java.util.Date类型
                    Date dt1 = new Date(yuce_time * 1000);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.tv_left:
                MyYuCeDetailActivity.this.finish();
                break;
