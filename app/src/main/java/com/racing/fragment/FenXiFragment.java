...
import com.racing.Adapter.FenXiAdapter;
...
import com.racing.racing.YuCeDetailActivity;
import com.racing.racing.ZhuanJiaActivity;
import com.racing.racing.ZhuanJiaDetailActivity;
...
public class FenXiFragment extends Fragment implements View.OnClickListener {
    ...
    private List<ZhuanJia> list_zhuanjia;
    //
    ...
    private void getDataZhuanjia() {
        OkHttpUtils
                .post()
                .url("http://www.zse6.com/index.php/mobile/pk10api/yuce_list")
            ...
            public void onResponse(String response, int id) {
                try {
                    ...
                    for (int i = 0; i < jsonObj.length(); i++) {
                        ...
                        String levela = o.getString("level");
                        List<Level> list_level = new ArrayList<>();
                        Level level = new Level();
                        JSONObject jsonObject = new JSONObject(levela);
                        String logo = jsonObject.getString("logo");
                        level.setLogo(logo);
                        list_level.add(level);
                        zhuanJia.setList_level(list_level);
                        zhuanJia.setTx(o.getString("tx"));
                        zhuanJia.setId(o.getInt("id"));
                        zhuanJia.setNicheng(o.getString("nicheng"));
                        list_zhuanjia.add(zhuanJia);
                    }
                    Message message = new Message();
                    message.what = 200;
                    myHandler.sendMessage(message);
    ...
    public Handler myHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 200:
                    try {
                        Glide.with(getActivity()).load(list_zhuanjia.get(0).getList_level().get(0).getLogo()).placeholder(new ColorDrawable(getActivity().getResources().getColor(R.color.color_content_bg))).//加载中显示的图片
                                error(R.drawable.img_user_tx)//加载失败时显示的图片
                                .into(img_vip1);
                        Glide.with(getActivity()).load(list_zhuanjia.get(1).getList_level().get(0).getLogo()).placeholder(new ColorDrawable(getActivity().getResources().getColor(R.color.color_content_bg))).//加载中显示的图片
                                error(R.drawable.img_user_tx)//加载失败时显示的图片
                                .into(img_vip2);
                        Glide.with(getActivity()).load(list_zhuanjia.get(2).getList_level().get(0).getLogo()).placeholder(new ColorDrawable(getActivity().getResources().getColor(R.color.color_content_bg))).//加载中显示的图片
                                error(R.drawable.img_user_tx)//加载失败时显示的图片
                                .into(img_vip3);
                        Glide.with(getActivity()).load(list_zhuanjia.get(3).getList_level().get(0).getLogo()).placeholder(new ColorDrawable(getActivity().getResources().getColor(R.color.color_content_bg))).//加载中显示的图片
                                error(R.drawable.img_user_tx)//加载失败时显示的图片
                                .into(img_vip4);
